...
def main():
    ...
    save_path = './data/twitter/stance_targets'
    ...
    for month_files_df in file_df.sort(['year', 'month', 'day'], descending=True).partition_by(['year', 'month', 'day']):
        try:
            ...
            unique_df = df.unique('rawContent').filter(pl.col('rawContent').is_not_null())
            docs = unique_df['rawContent'].to_list()
            ...
            target_df = unique_df.select(['id', 'rawContent']).with_columns(doc_df['Targets'])
            target_df = target_df.with_columns(pl.lit(finetune_kwargs).alias('finetune_kwargs'))

            df = df.select(['id', 'seed_id', 'date']).join(target_df, on='id', how='left')
